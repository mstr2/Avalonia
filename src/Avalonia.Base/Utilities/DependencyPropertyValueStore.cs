...
{
    /// <summary>
    /// Stores values with <see cref="DependencyProperty"/> as key.
    /// </summary>
    /// <typeparam name="TValue">Stored value type.</typeparam>
    internal sealed class DependencyPropertyValueStore<TValue>
    {
        private Entry[] _entries;

        public DependencyPropertyValueStore()
        {
        ...
        public bool TryGetValue(DependencyProperty property, out TValue value)
        {
            (int index, bool found) = TryFindEntry(property.GlobalIndex);
            if (!found)
            {
                value = default;
                return false;
        ...
        public void AddValue(DependencyProperty property, TValue value)
        {
            Entry[] entries = new Entry[_entries.Length + 1];

            for (int i = 0; i < _entries.Length; ++i)
            {
                if (_entries[i].PropertyId > property.GlobalIndex)
                {
                    if (i > 0)
                    {
                    ...
                    entries[i] = new Entry { PropertyId = property.GlobalIndex, Value = value };
        ...
        public void SetValue(DependencyProperty property, TValue value)
        {
            _entries[TryFindEntry(property.GlobalIndex).Item1].Value = value;
        }

        public Dictionary<DependencyProperty, TValue> ToDictionary()
        {
            var dict = new Dictionary<DependencyProperty, TValue>(_entries.Length - 1);

            for (int i = 0; i < _entries.Length - 1; ++i)
            {
                dict.Add(DependencyPropertyRegistry.Instance.FindRegistered(_entries[i].PropertyId), _entries[i].Value);
            }
